...
- Began and completed the first implementation pass for `#34`: `Graph` now exposes a minimal `pagerank()` method returning `indexed_lookup_map<NodeID, double>`, the repo also keeps a deprecated free-function alias for consistency with the other graph-local helpers, and the first test case checks that the returned scores stay normalized and rank a small directed graph sensibly.
