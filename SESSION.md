...
- Closed `#52` with a coverage-focused generated-doc pass: the semantic headers under `include/nxpp/` now carry explicit Doxygen `@file` briefs, the public graph alias/type presets now have clearer alias-level comments for the generated reference, and `docs/README.md` now records that this modular/header-level Doxygen audit has been completed. Updated `CHANGELOG.md` / `RELEASE_NOTES.md` to `1.0.7`.
- Closed `#53` by filling the remaining obvious Doxygen gaps on deprecated aliases and thin namespace-scope wrappers: traversal, shortest-path, components, spanning-tree, topological-sort, and flow compatibility aliases now expose short `@param` / `@return` coverage instead of only skeletal one-line stubs, and the release docs now track that pass under `1.0.8`.
- Closed `#54` by giving the generated docs a real module structure: lightweight Doxygen groups now mirror the semantic header split (`graph`, `attributes`, `multigraph`, `traversal`, `shortest paths`, `components`, `spanning/order`, `flow`, `centrality`, `generators`, `2-SAT`), and the generated main page now links to those groups directly. Updated `CHANGELOG.md` / `RELEASE_NOTES.md` to `1.0.9`.
- Closed `#55` by strengthening the generated landing page itself: `docs/DOXYGEN_MAINPAGE.md` now gives GitHub Pages readers a clearer start-here path, points directly to the most common result wrappers, and links back more explicitly to the companion markdown guides so the generated site behaves more like a usable API front door than a thin placeholder index. Updated `CHANGELOG.md` / `RELEASE_NOTES.md` to `1.0.10`.
- Closed `#57` by connecting the narrative docs and generated API docs more directly: `README.md` and the companion guides now link out to the most relevant published Doxygen entry pages and semantic module pages, `docs/README.md` now defines that cross-navigation rule explicitly, and the Doxygen landing page now points GitHub Pages readers back toward the longer guides when they need policy or curated explanation instead of only declaration-level detail. Updated `CHANGELOG.md` / `RELEASE_NOTES.md` to `1.0.11`.
- Closed `#58` by adding generated-doc-friendly examples for the richer result wrappers: the Doxygen declaration pages for `SingleSourceShortestPathResult`, `MaximumFlowResult`, `MinCostMaxFlowResult`, and `MinimumCutResult` now carry short embedded examples, the API guide now links directly to those generated pages, and the landing docs now mention that those struct pages are useful example entry points rather than only shape references. Updated `CHANGELOG.md` / `RELEASE_NOTES.md` to `1.0.12`.
- Closed `#56` with a presentation-only Doxygen / GitHub Pages polish pass: the generated site now enables the tree/sidebar navigation again, the local theme styles tables, fragments, and declaration pages more decisively, and `docs/DOXYGEN_MAINPAGE.md` now gives clearer reader-path guidance for first-time readers, module browsing, and wrapper-page discovery. Updated `CHANGELOG.md` / `RELEASE_NOTES.md` to `1.0.13`.
- Followed up immediately on the `#56` theme pass after reviewing the generated class/group pages: tightened the light-theme overrides for `memberdecls`, added more padding and left alignment for `Parameters` / `Template Parameters` / `Returns`, improved wrapping inside long declaration signatures, and visually de-emphasized the duplicated internal `requires` lines that Doxygen emits for some constrained helpers.
- Opened the next documented release-candidate bucket at `1.0.14` after publishing `v1.0.13`, then closed `#46` with a docs-only policy pass for implicit creation: the README, docs index, API architecture guide, and the core proxy comments now all state much more directly that write-style graph access may create missing nodes or edges, while read-style accessors and algorithmic reads do not create implicitly.
- Opened the next documented release-candidate bucket at `1.0.15`, then closed `#47` with a visibility-focused docs pass for `NodeID` requirements: the README now explains the practical `NodeID` contract up front, `docs/API_REFERENCE.md` now states explicitly that the core wrapper does not require hash support, the numeric-generator-only `std::size_t` construction rule is called out separately, and the main `Graph` declaration docs now repeat the same assumptions close to the template itself.
- Opened the next documented release-candidate bucket at `1.0.16`, then closed `#48` with a docs-only cleanup of the include-path and external-consumption story: the README now separates repository-local examples from normal external includes, `docs/EXTERNAL_USAGE.md` now distinguishes clone-local, modular external, vendored CMake, installed-package, and single-header paths more explicitly, and `docs/README.md` now points to that guide as the source of truth for consumption-mode guidance.
- Opened the next documented release-candidate bucket at `1.0.17`, then closed `#49` with a docs-only clarification pass on `"weight"` semantics: the README now states much earlier that `"weight"` refers to the built-in edge-weight property rather than an arbitrary edge-attribute key, `docs/API_REFERENCE.md` now repeats that rule prominently before the shortest-path and min-cost-flow tables, and `docs/README.md` now treats that narrow meaning as a high-value public topic instead of a buried lower-level caveat.
- Opened the next documented release-candidate bucket at `1.0.18`, then closed `#50` with a docs/process alignment pass for the single-header testing story: `docs/TEST.md` now distinguishes the dedicated `single-header.yml` smoke-check workflow from the deeper local `run_single_header_tests.sh` path, and it now states explicitly that the release workflow is the path that rebuilds and runs the standalone-header suite before publishing the tested release asset.
- Opened the next documented release-candidate bucket at `1.0.19`, then closed `#51` by upgrading the dedicated single-header CI itself: `single-header.yml` now rebuilds `dist/nxpp.hpp`, runs `bash scripts/unix/run_single_header_tests.sh`, still executes the small smoke program, and still uploads the generated artifact, so the dedicated workflow now validates the real standalone-header suite instead of stopping at a smoke test.
- Opened the next documented release-candidate bucket at `1.0.20`, then closed `#66` by realigning the in-repository package metadata to the documented release line: `CMakeLists.txt`, `conanfile.py`, and `packaging/vcpkg/ports/nxpp/vcpkg.json` were brought back into sync with the active documented release line instead of the stale `1.0.1` version, and `README.md` now states explicitly that the repository-hosted package metadata is aligned to the latest documented release line.
- Opened the next documented release-candidate bucket at `1.0.21`, then closed `#67` by adding a small release-metadata drift guard: `scripts/check_release_metadata_versions.py` now compares the top documented version against `CMakeLists.txt`, `conanfile.py`, and `packaging/vcpkg/ports/nxpp/vcpkg.json`, `release.yml` now runs that check before extracting release notes or publishing a release, and the repository-hosted package metadata itself was advanced to `1.0.21` so the guard passes against the current release candidate.
- Opened the next documented release-candidate bucket at `1.0.22`, then closed `#68` by fixing the multigraph capacity path inside `minimum_cut()`: `include/nxpp/flow.hpp` now reads capacities through precise `edge_id` lookup instead of the ambiguous endpoint-based `(u, v)` path, and `docs/API_REFERENCE.md` now says explicitly that heterogeneous parallel capacities are modeled per concrete edge instance in multigraph mode.
- Opened the next documented release-candidate bucket at `1.0.23`, then closed `#69` by adding a dedicated multigraph `minimum_cut()` regression: `tests/test_flow.cpp` now contains a small deterministic case with heterogeneous parallel capacities that checks both the aggregate cut value and the precise `cut_edge_ids` path, so the `#68` fix is pinned down by the formal assertion-based suite.
- Opened the next documented release-candidate bucket at `1.0.24`, then closed `#71` by adding dedicated formal centrality coverage: the new `tests/test_centrality.cpp` now checks deterministic `degree_centrality()`, `pagerank()`, and `betweenness_centrality()` results directly, and the Unix runner, Windows runner, and CMake formal-test list now all include that focused centrality test file.
- Opened the next documented release-candidate bucket at `1.0.25`, then folded the large-graph raw-Boost comparison lane into `compatibility.yml` as a dedicated `large-graph-compare` job: the heavy path stays separate from the fast formal-suite job, the old standalone `large-graph-compare.yml` workflow was removed, and the README/testing docs now describe large-graph comparison as part of the main compatibility workflow family.
- Audited open issue `#35` (benchmark harness) against current repository state and confirmed the acceptance criteria are now materially covered by existing assets (`scripts/run_benchmark_report.py`, `benchmark/BENCHMARK.md`, benchmark CSV outputs, and plots under `benchmark/imgs/`). Added a concise harness-status section to `benchmark/BENCHMARK.md` and prepared a closure-comment draft to keep issue handling explicit and low-overhead.
- Ran a README/docs alignment pass after the recent packaging work: updated the root release marker to `v1.0.19`, adjusted packaging-strategy wording so Conan and the repository-hosted vcpkg overlay are described as active paths (not only future steps), and added a short pointer to the ConanCenter-style submission recipe under `packaging/conan-center-index/`.
- Closed `#70` with a dedicated external-consumer integration layer: added fixture consumers under `tests/external_consumers/` (vendored CMake, installed-package CMake, and standalone single-header), added `scripts/unix/run_external_consumer_tests.sh` to execute those paths with mode-level failure visibility, added `.github/workflows/external-consumers.yml` as the dedicated CI lane, and updated `docs/TEST.md` so the testing story now explicitly includes that external-consumer verification surface.
- Started the AUR distribution evaluation as a concrete packaging path rather than a pure roadmap note: added a repository-hosted `packaging/aur/` skeleton with a release-based `PKGBUILD`, matching `.SRCINFO`, and a short local README, and updated the public docs to say explicitly that this is an evaluation skeleton for `nxpp` rather than an already-published AUR package.
- Opened the next documented release-candidate bucket at `1.0.27`, then closed `#72` by expanding selector-focused large-graph coverage beyond the single existing non-default case: `tests/test_large_graph_compare.cpp` now includes a second deterministic non-default selector regression (`boost::listS` / `boost::vecS`) that rechecks `remove_node()` alignment, weighted edge-state alignment, Dijkstra distance agreement, and attribute cleanup/preservation behavior against the raw Boost baseline; `docs/TEST.md` now states explicitly that two non-default selector regressions are covered.
- Opened the next documented release-candidate bucket at `1.0.28`, then closed `#73` by finalizing distribution-channel decisions as explicit policy: AUR is now documented as an active secondary release-based channel, FetchContent / CPM.cmake guidance is documented as the near-term CMake-consumer path, Homebrew tap and Spack are documented as later-stage candidates, and Meson WrapDB plus Debian / Ubuntu packaging are documented as deferred for now; the strategy text is now aligned across `README.md`, `docs/README.md`, `docs/EXTERNAL_USAGE.md`, and `packaging/aur/README.md`.
- Started serious wasm/node enablement for issue `#74`: installed/activated Emscripten locally, validated `em++` availability, identified the real local blocker as Boost header visibility under Emscripten host-include resolution, and updated all wasm build/test scripts to accept a `BOOST_INCLUDE` path (default `/usr/include`) so Node-compatible wasm bindings and wasm smoke/tests can compile without requiring runtime Boost installation for consumers.
- Performed a cleanup pass on the experimental wasm lane before broader API rollout: removed temporary smoke-only artifacts (`wasm/wasm_smoke.cpp`, `wasm/scripts/run_wasm_smoke.sh`, `wasm/scripts/run_wasm_node_api_smoke.sh`, `wasm/node/node_api_smoke.mjs`), simplified the wasm CI lane to run node-module build + formal wasm suite, and aligned `wasm/README.md`, `docs/TEST.md`, `wasm/node/README.md`, and `wasm/DEVELOPMENT_PLAN.md` to the new no-smoke baseline.
- Consolidated wasm documentation into a single source: replaced the old development-plan-only file with `wasm/WASM.md`, expanding it to cover scope, architecture, dependency model, commands, CI behavior, troubleshooting, phased rollout plan, risks, and definition of done; updated root/docs testing/index links to point to the new document.
- Expanded `wasm/WASM.md` with an explicit feature-tracking matrix and a strategy decision section for avoiding Emscripten-specific directives in canonical `include/` headers: the docs now recommend a wasm adapter/binding layer under `wasm/` to minimize native API drift while still extending Node-facing coverage incrementally.
- Implemented Sprint 1+2 contract hardening for the wasm Node lane: added explicit API contract-v0 documentation to `wasm/WASM.md`, introduced `wasm/node/node_api_contract_test.mjs` plus `wasm/scripts/run_wasm_node_contract_tests.sh`, integrated contract execution into `wasm/scripts/run_wasm_tests.sh`, and extended `.github/workflows/wasm-experimental.yml` to report dedicated node-contract status/output alongside node-build and wasm-suite results.
- Aligned wasm shortest-path semantics closer to the C++ result model per follow-up discussion: `DiGraph` now exposes `dijkstraShortestPaths(source)` returning a dedicated `DijkstraResult` object (`hasPathTo`, `distanceTo`, `pathTo`, `reachableNodes`), while the temporary compatibility aliases `dijkstraDistance` and `dijkstraPath` were intentionally removed from the Node surface; contract tests and `wasm/WASM.md` were updated accordingly.
- Realigned the wasm experimental lane after a folder refactor: moved node bindings to `wasm/node/nxpp_bindings.cpp`, moved Node contract tests to `wasm/test/node_api_contract_test.mjs`, switched build artifacts from `.tmp/wasm-node` to `wasm/build`, updated the runner/build scripts accordingly, and revalidated build + Node contract execution on the new layout.
- Opened the first experimental npm package lane for wasm consumption: aligned `wasm/package.json` to the published scoped package `@mik1810/nxpp-wasm`, standardized the package wrapper at `wasm/nxpp.mjs`, added `wasm/nxpp_example.js` for local smoke usage, tracked `wasm/build/nxpp_node.mjs` and `wasm/build/nxpp_node.wasm` via `.gitignore` exceptions, and validated package assembly with `npm pack --dry-run`.
- Strengthened the experimental wasm `graph.hpp` coverage in two staged slices: the Node-facing `DiGraph` contract now includes `neighbors`, `removeNode`, `removeEdge`, endpoint-based `getEdgeWeight`, plus the first edge-ID-aware core layer (`hasEdgeId`, `edgeIds`, `edgeIdsBetween`, `getEdgeEndpoints`, `getEdgeWeightById`, `setEdgeWeightById`), and the Node contract tests were extended accordingly; follow-up direction is now documented more explicitly as "finish `graph.hpp` first, then add the JS-side parity/proxy layer, then move on to later semantic headers."
- Refined the wasm public-API direction after the first `graph.hpp` slices: instead of exposing the full C++ alias matrix directly or collapsing all node IDs to strings, the docs now define a smaller NetworkX-like JS-facing graph family (`Graph`, `DiGraph`, `MultiGraph`, `MultiDiGraph`), keep the concrete C++ graph aliases internal to the wasm binding layer, and document that numeric and string node IDs should remain backed by distinct internal graph implementations selected lazily from first use rather than by one normalized representation.
- Reworked the experimental wasm `DiGraph` binding around that new node-ID model: the current Node-facing `DiGraph` now accepts integer-valued JS numbers and strings, selects its internal directed graph backend lazily on first use, preserves the chosen node-ID kind in `nodes()`, `neighbors()`, `DijkstraResult`, and `EdgeEndpoints`, and rejects mixed numeric/string node IDs in the same graph; the Node contract test now covers both numeric and string-backed cases plus mixed-ID failure behavior.
- Rebased the repository compatibility baseline onto Boost 1.74.0: `.github/workflows/compatibility.yml` now downloads the official Boost 1.74.0 archive and uses those headers for the GCC/Clang formal suite and the large-graph raw-Boost comparison lane, `README.md` / `docs/TEST.md` now state that 1.74.0 is the explicit compatibility floor, and a local GCC run against extracted Boost 1.74.0 headers advanced through the formal suite with many PASS results before hitting the repository's 30-second command timeout rather than an observed incompatibility.
- Completed the next `#74` stabilization/publish-prep slice: fixed current embind build diagnostics in `wasm/node/nxpp_bindings.cpp` (`::isfinite(...)` + explicit `std::variant` alternative construction), revalidated the wasm Node flow (`build_wasm_node_module.sh`, `run_wasm_node_contract_tests.sh`, and `node wasm/nxpp_example.js` all passing with emsdk active), created a ready-to-post issue update draft in `wasm/ISSUE_74_UPDATE_COMMENT.md`, and advanced `wasm/package.json` from `0.1.1` to `0.1.2`.
- Reverted the experimental Boost 1.74.0 compatibility-baseline change after the GitHub `large-graph-compare` lane did not pass on that baseline: `.github/workflows/compatibility.yml` is back to the previous runner/system-Boost setup, the public docs no longer claim 1.74.0 as an explicit floor, and the earlier 1.74.0 session note should now be treated as a superseded experiment rather than as the current compatibility policy.
- Extended the experimental wasm `DiGraph` core slice without leaving `graph.hpp`: the Node binding now exposes `addNodesFrom`, `addEdgeWithId`, `successors`, and `predecessors`, and the Node contract tests now cover batch node insertion, precise edge-id creation, and directed predecessor/successor behavior for both numeric and string-backed graphs.
- Added the first undirected public graph type to the experimental wasm surface: `Graph` now reuses the same lazy numeric/string backend selection model as `DiGraph`, the Node contract tests now cover undirected adjacency and shortest-path behavior, and the wasm docs now describe `Graph` / `DiGraph` as the current exported graph-core slice while `MultiGraph` / `MultiDiGraph` remain future work.
- Added the first exported multigraph wasm type: `MultiDiGraph` now reuses the same lazy numeric/string backend selection model as `Graph` / `DiGraph`, exposes the precise `removeEdgeById(edgeId)` path plus a first edge-ID-only typed edge-attribute layer (`string` and `number`), and the Node contract tests now cover parallel-edge identity, precise partial removal, bundle removal by endpoints, and per-edge typed metadata on both numeric and string-backed multigraphs.
- Completed a full wasm binding architecture refactor from a monolithic file to a module-aligned layout: replaced `wasm/node/nxpp_bindings.cpp` with a modular tree under `wasm/include/nxpp_wasm/` and `wasm/src/`, introduced the wasm umbrella header `wasm/include/nxpp_wasm.hpp`, and moved final Embind wiring to `wasm/src/nxpp_wasm.cpp` through `register_all_bindings()`.
- Added shared wasm binding utilities in `wasm/include/nxpp_wasm/common/` and `wasm/src/common/` (`js_convert`, `errors`, `binding_utils`) so JS boundary conversions and typed graph wrappers are centralized and reusable.
- Aligned graph-core exports with the `graph.hpp` type matrix and removed runtime backend switching entirely: the wasm Node layer now exports explicit typed classes (`GraphInt`, `GraphStr`, `DiGraphInt`, `DiGraphStr`, `MultiGraphInt`, `MultiGraphStr`, `MultiDiGraphInt`, `MultiDiGraphStr`) where each class owns one concrete backend.
- Kept graph API policy explicit in the new binding surface: simple graphs expose endpoint-based methods, while multigraphs additionally expose edge-ID APIs (`hasEdgeId`, `edgeIds`, `edgeIdsBetween`, `getEdgeEndpoints`, `getEdgeWeightById`, `setEdgeWeightById`, `removeEdgeById`).
- Updated the wasm build script to compile every source under `wasm/src/**/*.cpp` with `wasm/include/` on include path, updated the Node contract tests and wasm example to the new typed graph-core API, and revalidated locally with 30s-capped commands (`build_wasm_node_module.sh` PASS, `run_wasm_node_contract_tests.sh` PASS, `git diff --check` PASS).
- Added the first explicit TypeScript facade layer for the wasm package: introduced published facade artifacts in `wasm/dist/index.js` and `wasm/dist/index.d.ts`, introduced source organization under `wasm/ts/` (`types`, `load`, `core`, `internal`, and algorithm-module placeholders), and aligned the facade with explicit runtime constructors (`*Int` / `*Str`) plus generic TS interfaces (`Graph<T>`, `DiGraph<T>`, `MultiGraph<T>`, `MultiDiGraph<T>`).
- Updated wasm packaging metadata to expose the facade as the primary package surface (`main`/`exports`/`types` now target `dist/index.*`), kept `wasm/nxpp.mjs` as a compatibility re-export shim, and advanced the wasm package version from `0.1.2` to `0.1.3`.
- Updated documentation (`wasm/README.md`, `wasm/WASM.md`, and the wasm section in `README.md`) to describe the two-layer architecture and the new TS-facing layout.
- Adopted tracked facade artifacts in git: `.gitignore` now explicitly unignores `wasm/dist/` so compiled JS/TS facade outputs are versioned consistently with the package entrypoint contract.
- Hardened wasm package publication ordering so npmjs is always first and registry routing is deterministic: removed lifecycle-based chaining (`postpublish`) from `wasm/package.json`, made `publish:all` explicit (`publish:npm && publish:github`), and introduced token-free per-target npm config files (`wasm/.npmrc.publish-npm`, `wasm/.npmrc.publish-github`) to prevent host-level `@mik1810:registry` overrides from silently redirecting publish targets.
- Prepared the next experimental npm publish candidate by bumping `wasm/package.json` from `0.1.3` to `0.2.0` (and syncing `wasm/package-lock.json`), with publish-order docs now explicit in `README.md` and `wasm/README.md` for the deterministic npmjs-first flow.
- Aligned `RELEASE_NOTES.md` with the current top release line by adding the missing `1.0.35`/`1.0.34`/`1.0.33`/`1.0.32` sections so release-facing notes now match the current changelog head and the recent wasm refactor/facade/publish-order milestones.
- Completed real publication of `@mik1810/nxpp-wasm@0.2.0` on both npmjs and GitHub Packages with post-publish registry visibility checks, then added a root `TODO.md` continuation plan for the next implementation cycle while explicitly excluding items 6 (`topological_sort`), 11 (`generators`), and 12 (`sat`) from the active near-term scope.
- Refactored the wasm Node API contract test from one monolithic file into a small modular layout under `wasm/test/node_api_contract/`, keeping `wasm/test/node_api_contract_test.mjs` as the stable aggregate entrypoint so the existing shell runner and CI wiring did not need to change.
- Split the contract coverage into focused files for export surface, simple-graph behavior, multigraph behavior, and runtime type validation, and added extra graph-core edge-case checks around `clear()`, missing-edge mutation attempts, invalid edge IDs, and non-finite weights.
- During verification, the first refactor pass exposed that undirected `MultiGraph*` endpoint lookups still need uniqueness normalization for `edgeIdsBetween(...)`; corrected the test to preserve the pre-existing validated behavior there while keeping `MultiDiGraph*` checks exact.
- Revalidated the refactored Node contract suite successfully with `NXPP_WASM_NODE_CONTRACT_SKIP_BUILD=1 bash wasm/scripts/run_wasm_node_contract_tests.sh`, which returned `[WASM-NODE] contract-tests-ok`.
- Added a dedicated TypeScript compile-check file at `wasm/ts/compile_checks/facade_contract.ts` so the existing `npm --prefix wasm run build:types` step now statically exercises the graph-core facade shape: explicit runtime classes (`GraphInt`, `GraphStr`, `DiGraphInt`, `DiGraphStr`, `MultiGraphInt`, `MultiGraphStr`, `MultiDiGraphInt`, `MultiDiGraphStr`) are checked against the generic public interfaces (`Graph<T>`, `DiGraph<T>`, `MultiGraph<T>`, `MultiDiGraph<T>`) and the main graph-core methods are touched in typed call sites.
- Verified the new facade compile-check with `timeout 30s npm --prefix wasm run build:types`, which completed successfully.
- Closed the wasm `attributes` block for the explicit typed graph family. The shared binding layer in `wasm/include/nxpp_wasm/common/binding_utils.hpp` now exposes node-attribute and endpoint-based edge-attribute methods on `Graph*` / `DiGraph*` (`has*`, `get*`, `tryGet*`, `set*`, plus `getEdgeNumericAttr`) and adds the precise `edge_id` attribute path on `MultiGraph*` / `MultiDiGraph*` (`hasEdgeAttrById`, `getEdgeAttrById`, `tryGetEdgeAttrById`, `setEdgeAttrById`, `getEdgeNumericAttrById`).
- Aligned the TypeScript facade with the new runtime surface: `wasm/ts/types.ts`, `wasm/ts/internal/wasm_types.ts`, `wasm/ts/core/graph.ts`, and `wasm/ts/core/multigraph.ts` now include the full attributes contract, while `wasm/ts/core/attributes.ts` re-exports the shared `AttributeValue` type used by the facade.
- During implementation, a runtime-contract problem surfaced around `null` writes: the underlying C++ attribute storage does not preserve `nullptr_t` robustly through the existing normalization path, so the wasm public contract was tightened to `string | number | boolean` for writes, while `tryGet...` methods still use `null` as the "missing / unsupported" sentinel.
- Extended the modular Node contract suite with a dedicated `wasm/test/node_api_contract/attributes.test.mjs` file and widened the TS compile-check to exercise the new attribute APIs end-to-end.
- Revalidated the finished wasm `attributes` block with 30-second-capped commands: `timeout 30s npm --prefix wasm run build:types` PASS, `timeout 30s bash wasm/scripts/build_wasm_node_module.sh` PASS, and `timeout 30s env NXPP_WASM_NODE_CONTRACT_SKIP_BUILD=1 bash wasm/scripts/run_wasm_node_contract_tests.sh` PASS.
- Closed the wasm `traversal` block for the explicit typed graph family. The shared binding layer now exports `bfsEdges`, `bfsTree`, `bfsSuccessors`, `dfsEdges`, `dfsTree`, `dfsPredecessors`, and `dfsSuccessors` on both simple graphs and multigraphs, mapping the C++ traversal results into explicit JS DTOs instead of trying to mirror the native visitor/proxy surface.
- Chose a result-oriented JS mapping for traversal trees and sparse maps: tree-returning methods now expose `{ nodes, edges }`, BFS/DFS edge lists return ordered `{ source, target }` entries, BFS/DFS successors return sparse `{ node, successors }` entries, and DFS predecessors return sparse `{ node, predecessor }` entries. This preserves the content and ordering of the base traversal helpers without introducing a fragile nested-wrapper graph return type into the wasm bridge.
- Aligned the TypeScript facade and compile-check with the new traversal surface by adding traversal result types plus method coverage in `wasm/ts/types.ts`, `wasm/ts/internal/wasm_types.ts`, `wasm/ts/core/graph.ts`, `wasm/ts/core/multigraph.ts`, `wasm/ts/algorithms/traversal.ts`, and `wasm/ts/compile_checks/facade_contract.ts`.
- Extended the modular Node contract suite with `wasm/test/node_api_contract/traversal.test.mjs`, covering BFS/DFS edge lists, tree DTOs, sparse successor/predecessor results, singleton behavior, multigraph traversal support, and standardized missing-start traversal failures.
- The first traversal-tree bridge pass exposed a string-node corruption issue when routing through an intermediate C++ tree wrapper. Corrected the wasm bridge to build the public `{ nodes, edges }` tree DTOs directly from `start + traversal edge list`, then revalidated successfully with 30-second-capped commands: `timeout 30s npm --prefix wasm run build:types` PASS, `timeout 30s bash wasm/scripts/build_wasm_node_module.sh` PASS, and `timeout 30s env NXPP_WASM_NODE_CONTRACT_SKIP_BUILD=1 bash wasm/scripts/run_wasm_node_contract_tests.sh` PASS.
- Per follow-up cleanup, removed the misleading empty `register_attributes_bindings()` and `register_traversal_bindings()` hooks. The module-aligned Embind wiring now lives only in `wasm/src/attributes.cpp` and `wasm/src/traversal.cpp` as concrete per-class binder functions consumed by `wasm/src/graph.cpp` and `wasm/src/multigraph.cpp`, while `wasm/include/nxpp_wasm/register_all.hpp` no longer pretends those modules self-register independently.
- Revalidated that structural cleanup with user-run commands: `bash wasm/scripts/build_wasm_node_module.sh` PASS and `env NXPP_WASM_NODE_CONTRACT_SKIP_BUILD=1 bash wasm/scripts/run_wasm_node_contract_tests.sh` PASS.
- Started the wasm `shortest_paths` block with the single-pair and single-source families. The binding layer now exposes `shortestPath*`, `dijkstra*`, `bellmanFord*`, and `dagShortestPaths(...)` on the explicit typed graph family, while keeping the weighted wrappers pinned to the built-in `"weight"` channel instead of promising arbitrary attribute-key weighting in JS.
- Added a JS-facing single-source shortest-path result wrapper in the TS facade: `dijkstraShortestPaths(source)`, `bellmanFordShortestPaths(source)`, and `dagShortestPaths(source)` now return `{ distance, predecessor, hasPathTo(target), pathTo(target) }`, and `dijkstraPathLengths(source)` exposes the sparse distance table directly.
- Extended the modular Node contract suite with `wasm/test/node_api_contract/shortest_paths.test.mjs`, covering unweighted shortest path, weighted shortest path, Dijkstra single-source reconstruction, Bellman-Ford on negative weights, DAG shortest paths on `MultiDiGraph*`, disconnected-path failures, and rejection of unsupported weighted key names.
- Extended the TypeScript compile-check in `wasm/ts/compile_checks/facade_contract.ts` so the explicit runtime classes are statically checked against the new shortest-path surface on `Graph<T>`, `DiGraph<T>`, `MultiGraph<T>`, and `MultiDiGraph<T>`.
- Updated `wasm/README.md` and `wasm/WASM.md` to describe the newly exposed shortest-path surface and to clarify that the remaining open part of the `shortest_paths.hpp` block is the `all-pairs` `floyd_warshall` family.
- User-run verification for this shortest-path pass succeeded with `wasm/scripts/build_wasm_node_module.sh` PASS, `npm --prefix wasm run build:types` PASS, and `env NXPP_WASM_NODE_CONTRACT_SKIP_BUILD=1 bash wasm/scripts/run_wasm_node_contract_tests.sh` PASS.
- Closed the remaining wasm `shortest_paths` work by exposing the `all-pairs` Floyd-Warshall family on the explicit typed graph classes: `floydWarshallAllPairsShortestPaths()` now returns a dense `number[][]` matrix in stable node order, and `floydWarshallAllPairsShortestPathsMap()` now returns serializable `{ source, distances: [{ target, distance }] }` DTO entries instead of a JS `Map`.
- Extended the TS facade and compile checks for the all-pairs surface across `Graph<T>`, `DiGraph<T>`, `MultiGraph<T>`, and `MultiDiGraph<T>`, and extended the Node contract tests with dense-matrix / DTO-map assertions plus a multigraph parallel-edge regression for Floyd-Warshall.
- Updated `wasm/README.md` and `wasm/WASM.md` again so the docs now treat `shortest_paths.hpp` as covered in full for the current wasm lane rather than leaving Floyd-Warshall listed as still pending.
- User-run verification for the final all-pairs shortest-path pass succeeded with `bash wasm/scripts/build_wasm_node_module.sh` PASS, `npm --prefix wasm run build:types` PASS, and `env NXPP_WASM_NODE_CONTRACT_SKIP_BUILD=1 bash wasm/scripts/run_wasm_node_contract_tests.sh` PASS. The build step took longer than the repo's 30-second autonomous-command budget, so it remained user-run only.
- Prepared the next wasm publish candidate by bumping `wasm/package.json` from `0.2.0` to `0.3.0` and syncing `wasm/package-lock.json`, matching the now-completed `shortest_paths` module coverage in the experimental Node/TS package lane.
- Investigated the local post-Codespaces move to WSL after VS Code started falling back from `/workspaces/emsdk/upstream/emscripten/em++` to `/usr/bin/gcc`. Confirmed the regression was a repository-local IntelliSense config issue rather than a recent wasm code change, updated `.vscode/c_cpp_properties.json` to resolve `em++` from `PATH` instead of a hardcoded Codespaces path, refreshed `wasm/WASM.md` to document a generic `emsdk_env.sh` activation flow plus the current WSL-local example path (`$HOME/.local/opt/emsdk/emsdk_env.sh`), bootstrapped a local `emsdk` clone under `~/.local/opt/emsdk`, and observed that `./emsdk install latest` timed out under the repo's 30-second autonomous-command limit while downloading the full SDK payload, so final toolchain activation remains a manual/local follow-up step.
- Corrected `wasm/nxpp_example.js` after the package facade moved fully to ESM: the JavaScript smoke example now imports the default package facade with `import nxpp from "@mik1810/nxpp-wasm"` instead of using stale CommonJS output, and `timeout 30s node wasm/nxpp_example.js` passes with the expected neighbor and weight output.
- Updated both wasm smoke examples (`wasm/nxpp_example.js` and `wasm/nxpp_example.ts`) to use a slightly richer weighted directed graph and to demonstrate `dijkstraShortestPaths(1)` through `pathTo(5)` plus the single-source distance table; `timeout 30s node wasm/nxpp_example.js` passes locally, while `timeout 30s npm --prefix wasm run build:types` is currently blocked by the WSL shell resolving `npm` to the Windows `CMD.EXE` wrapper instead of a Linux npm executable.
- Tightened `wasm/nxpp_example.ts` so the TypeScript smoke example explicitly imports and uses the public facade types for `SingleSourceShortestPathResult<number>` and `ShortestPathDistanceEntry<number>[]`, making the Dijkstra example demonstrate both runtime usage and the typed TS surface.
- Prepared the release/history docs for the wasm example and publish-routing cleanup under `1.0.38`: `CHANGELOG.md` and `RELEASE_NOTES.md` now describe the refreshed Dijkstra examples, moved wasm continuation plan, direct publish registry scripts, and WSL-friendly Emscripten activation docs, while `README.md` now points at `wasm/TODO.md` and `wasm/package.json` instead of deleted root/publish-routing files.
- Correction to the previous TypeScript-example note: the committed `wasm/nxpp_example.ts` uses the public `DiGraph<number>` and `SingleSourceShortestPathResult<number>` types directly, while the distance table is printed from `dijkstra.distance` without an explicit local `ShortestPathDistanceEntry<number>[]` variable.
- Updated the wasm package smoke examples after local npm dependency restoration on WSL: `wasm/nxpp_example.js` and `wasm/nxpp_example.ts` now import `DiGraphInt` directly from `@mik1810/nxpp-wasm`, `timeout 30s node wasm/nxpp_example.js` passes, and `timeout 30s npm --prefix wasm run build:types` passes after `npm install` restored the local TypeScript toolchain.
- Started and implemented the wasm `spanning_tree.hpp` block for the explicit typed graph family: added Embind methods for `kruskalMinimumSpanningTree()` and `primMinimumSpanningTree(root)`, mapped both to serializable `{ source, target }` edge DTO arrays, extended the TypeScript facade / compile checks / Node contract tests, updated wasm docs and release notes, and advanced the experimental wasm package candidate to `0.4.0`. Verification now passes with `bash wasm/scripts/build_wasm_node_module.sh`, `timeout 30s env NXPP_WASM_NODE_CONTRACT_SKIP_BUILD=1 bash wasm/scripts/run_wasm_node_contract_tests.sh`, `timeout 30s npm --prefix wasm run build:types`, `timeout 30s node wasm/nxpp_example.js`, and `timeout 30s git diff --check`.
- Implemented issue `#75` for explicit wasm facade lifetime management: all public graph facade classes now expose idempotent `dispose()`, attach `Symbol.dispose` when the runtime provides it, and route every operation through a raw-object getter that throws `WASM graph operation failed: graph has been disposed.` after disposal. The Node contract suite now covers simple graph and multigraph disposal behavior, TypeScript compile checks exercise `dispose()`, docs/release notes were updated, and the experimental wasm package candidate advanced to `0.5.0`. Verification passes with `timeout 30s npm --prefix wasm run build:types`, `timeout 30s env NXPP_WASM_NODE_CONTRACT_SKIP_BUILD=1 bash wasm/scripts/run_wasm_node_contract_tests.sh`, `timeout 30s node wasm/nxpp_example.js`, and `timeout 30s git diff --check`.
- Implemented issue `#79` for wasm TypeScript facade runtime error mapping: raw Embind/C++ calls now pass through a proxy boundary that normalizes catchable failures to `WASM graph operation failed: ...`, while facade pre-validation makes common missing-node, missing-edge, missing-attribute, traversal-start, shortest-path unreachable, weight-key, and multigraph edge-id failures deterministic JavaScript `Error` cases. Added Node contract coverage for normalized messages, updated docs/release notes, and advanced the experimental wasm package candidate to `0.6.0`. Verification passes with `timeout 30s npm --prefix wasm run build:types`, `timeout 30s env NXPP_WASM_NODE_CONTRACT_SKIP_BUILD=1 bash wasm/scripts/run_wasm_node_contract_tests.sh`, `timeout 30s node wasm/nxpp_example.js`, and `timeout 30s git diff --check`.
- Published `@mik1810/nxpp-wasm@0.6.0` to npmjs and GitHub Packages. The first `npm run publish:all` attempt stopped on npmjs `EOTP`, the npmjs publish was completed manually through npm's browser authentication flow, then GitHub Packages was published with `npm publish --registry=https://npm.pkg.github.com/`. Registry checks confirmed `0.6.0` is visible on both npmjs and GitHub Packages.
- Closed issue `#76` with a real npm tarball consumer verification lane for the experimental wasm package: added `wasm/scripts/run_npm_pack_consumer_test.sh` to build (or reuse) the Node wasm module, run `npm pack`, install the generated tarball into an isolated fixture consumer, and execute an ESM smoke test through the published package entrypoint.
...
- Closed issue `#81` with a landing-page simplification pass on the root `README.md`: reduced the root document to overview, quick start, installation summary, feature overview, support-status table, docs map, and concise WASM status; moved deep packaging policy, caveats, and operational details to the existing companion docs via explicit links (`docs/EXTERNAL_USAGE.md`, `docs/API_REFERENCE.md`, `docs/TEST.md`, `wasm/README.md`, and `wasm/WASM.md`). Updated release/history docs for this slice under `1.0.43` in `CHANGELOG.md` and `RELEASE_NOTES.md`.
- Closed issue `#86` by documenting the `remove_node` hot path: added a Doxygen `@warning` to `include/nxpp/graph.hpp`, added a node-removal/performance section to `docs/API_REFERENCE.md` with an anti-pattern example and a rebuild-based alternative, and updated the `remove_node` table row. Aligned the core mutation table row and release/history docs to `1.0.44` in `CHANGELOG.md` and `RELEASE_NOTES.md`. The root `README.md` was later kept free of a prominent `remove_node` callout so the hot-path text stays in the API guide.
- Closed issue `#82` by introducing `docs/STABILITY.md` as the canonical support and API stability matrix for C++ surfaces, packaging channels, generated reference, and experimental wasm, and by slimming the root `README.md` down to a short pointer plus docs index wiring in `docs/README.md`. Updated release/history docs to `1.0.45` in `CHANGELOG.md` and `RELEASE_NOTES.md`.
- Closed issue `#89` with `Graph::to_dot` / `to_dot_string` in `include/nxpp/graph.hpp`, a `test_core` regression, `snippet/dot` parity files, and API guide / root README touch-ups; advanced release history to `1.0.46` in `CHANGELOG.md` and `RELEASE_NOTES.md`.
- Added a local `demo/print_graph/` example that builds a small weighted directed graph with Italian city names as `std::string` node IDs, writes DOT through `to_dot(...)`, and generates an SVG with Graphviz `dot`; while using the demo, added the missing explicit string-based weighted aliases `WeightedDiGraphStr`, `WeightedMultiGraphStr`, and `WeightedMultiDiGraphStr` so the alias matrix is symmetric with the existing `Int` names while preserving the older `WeightedDiGraph` / `WeightedMultiGraph` / `WeightedMultiDiGraph` compatibility aliases.
- Adjusted the local `demo/print_graph/` example so generated weighted edges also carry a DOT `label` attribute with the same numeric value as the built-in `weight`; this keeps the demo-specific SVG output readable because Graphviz uses `weight` for layout but only renders `label` as visible edge text.
- Closed issue `#91` by adding `include/nxpp/viz/dot.hpp` with `nxpp::viz::DotOptions`, `to_dot(...)`, and `write_dot(...)`, including it from `include/nxpp.hpp`, making weighted DOT export emit both `weight=...` and visible `label=...` values, adding focused `test_core` coverage, updating `snippet/dot`, moving the print-graph demo from `demo/` to `examples/print_graph/`, and advancing release docs to `1.2.0` in `README.md`, `CHANGELOG.md`, and `RELEASE_NOTES.md`.
- Followed up on the new `nxpp::viz` DOT exporter and the existing member DOT convenience path by making built-in weighted-edge attribute values quote themselves when they are not plain DOT identifiers or numeric values, so string weights with spaces produce valid DOT. Added direct `test_core` coverage for quoted string weights; `timeout 30s g++ ... tests/test_core.cpp ... && timeout 30s /tmp/nxpp_test_core`, `timeout 30s bash scripts/unix/test_single_snippet.sh dot`, and `timeout 30s git diff --check` pass.
- Removed the older member-level DOT convenience APIs `Graph::to_dot(std::ostream&)` and `Graph::to_dot_string()` after deciding that the supported public surface should be the new `nxpp::viz` namespace only. Kept the release candidate at `1.2.0` rather than moving to `2.0.0`, and aligned `CMakeLists.txt`, `conanfile.py`, and the repository vcpkg port metadata to `1.2.0` so the release metadata drift guard matches the top documented version.
- Removed `snippet/dot/` from the `nxpp::viz` branch after deciding not to maintain a DOT snippet lane; the DOT exporter remains covered by focused `test_core` cases and the renderable `examples/print_graph/` example.
- Fixed the Windows/MSVC failure in `test_viz_write_dot_file`: the test now destroys the `std::ifstream` before calling `std::filesystem::remove(path)`, avoiding Windows' open-file deletion error while preserving the same DOT file content assertion.
- Started issue `#90` and implemented `Graph::subgraph(nodes)` as a materialized node-induced subgraph API. The implementation validates requested nodes, ignores duplicate requested IDs, returns the same graph template type, copies selected node attributes and induced edge attributes, preserves built-in weights and multigraph parallel edge instances with fresh edge IDs, and rejects missing nodes with `std::invalid_argument`. Added focused `test_core` coverage and advanced the release candidate/package metadata to `1.3.0`.
- Continued issue `#90` cleanup after the interrupted session. Fixed the C++ `subgraph()` implementation so it copies induced edges through internal graph state instead of depending on `add_edge_with_id()` definitions from `multigraph.hpp`, which keeps `include/nxpp/graph.hpp` usable on its own. Completed the WASM TypeScript facade by adding `subgraph(nodes)` to multigraph wrappers, regenerated `wasm/dist`, and documented the new WASM method in `wasm/README.md` and `wasm/WASM.md`. Verified focused C++ smoke programs and `npm --prefix wasm run build:types`; full WASM runtime build and contract checks are blocked locally because `em++` is not on `PATH`.
- Follow-up verification note for issue `#90`: `env NXPP_WASM_NODE_CONTRACT_SKIP_BUILD=1 bash wasm/scripts/run_wasm_node_contract_tests.sh` passes against the existing local artifact after loading NVM. Only rebuilding the WASM runtime remains blocked by missing `em++`.
- Added Node API contract coverage for WASM `subgraph(nodes)` on simple graphs and multigraphs, including induced-edge filtering, attribute copying, missing-node rejection, and parallel-edge preservation. After loading emsdk explicitly, the previously failing `wasm/src/attributes.cpp` compile path succeeds, confirming the `NodeJsPolicy` ordering fix in `binding_utils.hpp`; the full `build_wasm_node_module.sh` still exceeds the required 30-second command limit before producing a refreshed artifact.
- User ran the full WASM rebuild for issue `#90`; `bash wasm/scripts/build_wasm_node_module.sh` completed successfully and refreshed `wasm/build/nxpp_node.mjs` plus `wasm/build/nxpp_node.wasm`. The follow-up `env NXPP_WASM_NODE_CONTRACT_SKIP_BUILD=1 bash wasm/scripts/run_wasm_node_contract_tests.sh` passed, including the new `subgraph(nodes)` runtime contract coverage.
- Started issue `#111` after reviewing the open issue list with the GitHub CLI now available locally. Implemented the literal-zero guard in `nxpp::to_2sat_vertex_id`, documenting that literal `0` is invalid in 2-SAT and now throws `std::invalid_argument` instead of producing vertex index `-1`.
- Added focused `test_edge_cases` coverage for valid 2-SAT literal mapping and literal-zero rejection, updated the API reference and release/history docs, and advanced the C++ release candidate metadata from `1.3.1` to `1.3.2`.
- Continued with issue `#106` in the same working tree. Simplified the duplicated missing-node branch in `Graph::node(...)` from separate `!Multi` and fallback branches to one `if (!has_node(u)) add_node(u);`, preserving the documented implicit node-creation policy.
- Updated release/history docs and repository-hosted package metadata from `1.3.2` to `1.3.3` so the new top release candidate includes both `#111` and `#106`.
- User requested that corrected issues be closed immediately after verification and that future fixes explicitly check whether declared asymptotic complexities are affected. Closed GitHub issues `#111` and `#106` with comments after their local fixes and test verification.
- Started issue `#108`. `erdos_renyi_graph(n, p, seed)` now pre-creates node IDs `0..n-1` before sampling candidate edges, so isolated nodes are preserved even for `p = 0.0`. This adds lower-order node insertion work and does not change the dominant pair-sampling order of the generator. Added focused `test_edge_cases` coverage and advanced release/history metadata to `1.3.4`.
- Closed GitHub issue `#108` after test verification, with a note that the added pre-creation step does not change the dominant asymptotic order.
- Started issue `#112`. Updated `bfs_tree(...)` and `dfs_tree(...)` to materialize `Graph<NodeID, EdgeWeight, Directed>` instead of hardcoding `double` for traversal-tree edge weights. Added compile-time regression coverage with `Graph<int, int, true>`. This changes only the returned graph type, not the traversal algorithm or amount of traversal work, so the documented `O(n + m)` complexity remains valid. Advanced release/history metadata to `1.3.5`.
- Closed GitHub issue `#112` after test verification, with a note that traversal-tree complexity remains `O(n + m)`.
- Started issue `#110`. Endpoint-based removal in undirected multigraphs now uses a matching-edge predicate to remove every BGL edge between the two endpoint descriptors, including parallel edges inserted as `(v, u)`. This avoids leaving live BGL edges after wrapper edge metadata has been erased. The broader scan is limited to the affected `!Directed && Multi` case; simple graphs and directed graphs keep the previous removal path. No public complexity table currently declares a tighter `remove_edge(u, v)` bound, but the undirected multigraph repair is `O(E)` in that case. Advanced release/history metadata to `1.3.6`.
- Closed GitHub issue `#110` after test verification, with a note that the `O(E)` matching-edge removal path is limited to undirected multigraphs.
- Started issue `#113`. Flow capacity setup now reads capacity attributes directly from the stored `std::any` value and validates that capacities are non-negative integral values representable as `long`, instead of converting through `double` and truncating silently. Fractional and out-of-range capacities now throw during flow setup. Added focused `test_flow` coverage and advanced release/history metadata to `1.3.7`. The validation is constant work per edge during the existing auxiliary flow-graph construction, so the documented flow setup complexity remains unchanged.
- Closed GitHub issue `#113` after test verification, with a note that capacity validation keeps the documented `O(n + m + F...)` flow bounds unchanged.
- Started issue `#114`. After discussing the tradeoff between instance-local staged state and a synchronized static cache, chose the mutex approach to avoid adding per-graph storage overhead. The staged min-cost-flow cache and invalidation set are now protected by a per-graph-specialization mutex across invalidate, clear, push/store, and cycle-canceling access. Added a regression that staged min-cost-flow states remain isolated between two graph instances. Lock/unlock is constant overhead per staged-flow call and does not affect the asymptotic flow bounds. Advanced release/history metadata to `1.3.8`.
- Closed GitHub issue `#114` after test verification, with a note that synchronized staged-flow cache access keeps the documented flow bounds unchanged.
- Started issue `#107`. `complete_graph(...)` now preserves ordered-pair generation for directed graphs but emits each unordered pair only once for undirected graphs, avoiding unintended parallel edges in undirected multigraphs. Added regression coverage for simple undirected, directed, and undirected multigraph complete-graph edge counts. The asymptotic generator cost remains `O(n^2)`, while undirected graphs now do half as many candidate insertions. Advanced release/history metadata to `1.3.9`.
- Closed GitHub issue `#107` after test verification, with a note that complete-graph generation remains `O(n^2)` while undirected graphs now do fewer candidate insertions.
- Started issue `#109`. Added a shared guarded predecessor-chain reconstruction helper for eager source-target shortest-path helpers and routed unweighted `shortest_path(...)`, `dijkstra_path(...)`, and `bellman_ford_path(...)` through it. The guard uses a hop counter bounded by the predecessor vector size, not a hash-based visited set, so reconstruction remains `O(V)` and cannot loop forever on inconsistent predecessor chains. Advanced release/history metadata to `1.3.10`.
- User merged draft PR `#146` into `main`; rebased the `#120` documentation fix onto the updated `main` and prepared it as release candidate `1.3.11`. Corrected the inline `connected_components()` documentation in `include/nxpp/graph.hpp` to describe the actual node-to-component-label return value instead of a component count. No code path or asymptotic complexity changed.
- Started issue `#144`. Updated DOT attribute numeric detection so a leading plus sign is not accepted as a DOT number, while preserving leading minus and exponent signs. Added `test_core` regression coverage for plus-signed string weights emitted as quoted DOT attribute values. Advanced release/history metadata to `1.3.12`; the change is linear in the attribute string length as before and does not affect graph-level asymptotic complexity.
- Closed GitHub issue `#144` after verification with `git diff --check`, `python3 scripts/check_release_metadata_versions.py`, and `bash scripts/unix/run_tests.sh` (`62/62`).
- Started issue `#140`. Updated both Conan recipes to copy `cmake/nxppConfig.cmake.in` into the package `cmake/` directory, matching the `cmake/*` source export with package contents. Advanced release/history metadata to `1.3.13`. This is packaging-only and does not affect runtime behavior or computational complexity.
- Closed GitHub issue `#140` after verification with Python recipe compilation, `git diff --check`, release metadata check, external CMake consumer tests, and the full C++ suite (`62/62`). After the user installed Conan locally, `conan create . --profile:build=default --profile:host=default -s:h compiler.cppstd=20` also passed for the root recipe and the generated package was confirmed to contain `cmake/nxppConfig.cmake.in`.
- Started issue `#145`. Documented staged min-cost-flow thread-safety boundaries in the root README, API reference, and public Doxygen comments: the cache container is synchronized, but concurrent mutation or staged-flow sequences on the same graph instance require external synchronization. Advanced release/history metadata to `1.3.14`. This is docs-only and does not affect runtime behavior or computational complexity.
- Closed GitHub issue `#145` after verification with `git diff --check`, release metadata check, and the full C++ suite (`62/62`).
- Started issue `#136`. Added `include/nxpp/numeric.hpp` beside the other public module headers and moved the duplicated numeric `std::any` conversion chain from `attributes.hpp` and `multigraph.hpp` into a shared helper. The shared conversion now recognizes signed integers, unsigned integers, floating-point values, and `long double`; `std::size_t` is supported through its platform unsigned integer type. Added focused attribute tests for endpoint and multigraph edge-id numeric lookups. Advanced release/history metadata to `1.3.15`. Complexity remains unchanged because each numeric lookup still performs a constant-length `std::any_cast` dispatch chain with no hash-based containers. Verification passed with `git diff --check`, release metadata check, focused attribute/multigraph tests, the full C++ suite (`64/64`), and single-header tests (`61/61`).
- Started issue `#126`. Extracted shared Floyd-Warshall matrix/order construction so `floyd_warshall_all_pairs_shortest_paths_map()` no longer sorts node order and then calls the matrix API, which sorted again. Added focused `test_core` coverage that matrix and map views remain aligned. Advanced release/history metadata to `1.3.16`. The dominant bound remains `O(n^3)`; the map wrapper removes a duplicate `O(n log n)` sort and still pays lower-order ordered-map materialization. Verification passed with `git diff --check`, release metadata check, focused `test_core`, the full C++ suite (`65/65`), and single-header tests (`62/62`).
- Started issue `#117`. Added `Graph::num_edges()` as a direct `boost::num_edges(g)` wrapper plus the matching deprecated free-function alias. Added focused simple-graph and multigraph regression coverage, documented the `O(1)` edge-count query in the API and complexity guides, and advanced release/history metadata to `1.3.17`. This avoids the `O(m)` edge-vector materialization required by `edges().size()` and introduces no hash-based containers.
- Closed GitHub issue `#117` after verification with `git diff --check`, release metadata check, focused `test_core` and `test_multigraph` runs, the full C++ suite (`67/67`), and single-header tests (`64/64`).
- Started issue `#143`. Chose to wire the existing public `DotLayout` enum into `DotOptions` instead of removing it, preserving compatibility while eliminating the dead API surface. DOT export now emits an optional Graphviz `layout=...` graph attribute when requested, default output remains unchanged, focused `test_core` coverage was added, and release/history metadata advanced to `1.3.18`. This adds only constant formatting work when the option is set and does not affect graph algorithm complexity.
- Verified issue `#143` locally with `git diff --check`, `python3 scripts/check_release_metadata_versions.py`, and focused `ctest --test-dir build -R '^test_core$' --output-on-failure`; all passed.
- Closed GitHub issue `#143` after local verification.
- Started issue `#141`. Documented the intentional global Boost namespace property extension for `vertex_wrapper_index_t` beside the declaration and added a compile-time check that BGL installed it as a vertex property. Advanced release/history metadata to `1.3.19`. This is compile-time maintainability only and does not affect runtime behavior or computational complexity.
- Verified issue `#141` locally with `git diff --check`, `python3 scripts/check_release_metadata_versions.py`, and focused `ctest --test-dir build -R '^test_core$' --output-on-failure`; all passed.
- Closed GitHub issue `#141` after local verification.
- Started issue `#142`. Added backward-compatible DOT options for graph-level attributes and opt-in user node/edge attribute emission. Default DOT output remains stable because stored user attributes are emitted only when `show_user_attrs` is true. Added focused `test_core` coverage for graph, node, and edge attributes plus quoting. Advanced release/history metadata to `1.3.20`. The feature adds linear formatting work over emitted attributes and does not change graph algorithm complexity.
- Verified issue `#142` locally with `git diff --check`, `python3 scripts/check_release_metadata_versions.py`, focused `ctest --test-dir build -R '^test_core$' --output-on-failure`, and `bash scripts/unix/run_tests.sh` (`69/69`); all passed.
- Closed GitHub issue `#142` after local verification.
- Started issue `#121`. Chose to preserve the existing Boost-style `root -> root` entry in `prim_minimum_spanning_tree(root)` for compatibility and documented that callers should skip it when interpreting the parent map as tree edges. Added focused `test_core` coverage for the root self-entry and advanced release/history metadata to `1.3.21`. This is documentation plus regression coverage only and does not affect runtime behavior or computational complexity.
- Verified issue `#121` locally with `git diff --check`, `python3 scripts/check_release_metadata_versions.py`, and focused `ctest --test-dir build -R '^test_core$' --output-on-failure`; all passed.
- Closed GitHub issue `#121` after local verification.
- Started issue `#118`. Preserved the existing lookup wrapper APIs and documented that read-style `operator[]` access throws like `at()` on missing keys, including `indexed_lookup_map` where `operator[]` never inserts. Added focused `test_core` coverage for missing-key exceptions and advanced release/history metadata to `1.3.22`. This is API documentation plus regression coverage only and does not affect algorithmic complexity.
- Verified issue `#118` locally with `git diff --check`, `python3 scripts/check_release_metadata_versions.py`, and focused `ctest --test-dir build -R '^test_core$' --output-on-failure`; all passed.
- Closed GitHub issue `#118` after local verification; the first close attempt hit a GitHub 504 and the retry succeeded.
- Started issue `#116`. Kept mutable `Graph::operator[]` write-creates semantics for compatibility and added const proxy access that never creates missing nodes, throwing on a missing source node through a checked read path. Documented the mutable/const split and added focused `test_edge_cases` coverage. Advanced release/history metadata to `1.3.23`. The change adds a constant-time checked lookup on const proxy entry and does not affect algorithmic complexity.
- Verified issue `#116` locally with `git diff --check`, `python3 scripts/check_release_metadata_versions.py`, focused `ctest --test-dir build -R '^test_edge_cases$' --output-on-failure`, and `bash scripts/unix/run_tests.sh` (`72/72`); all passed.
- Closed GitHub issue `#116` after local verification.
- Started issue `#119`. Added explicit `WeightMode` overloads for source-target shortest-path helpers so callers can choose `Unweighted` or `BuiltIn` without using the compatibility string selector. Kept existing string overloads compatible, including rejection of unsupported custom keys, and documented the new typed API. Advanced release/history metadata to `1.3.24`. The overloads delegate to existing algorithms, so asymptotic complexity is unchanged.
- Verified issue `#119` locally with `git diff --check`, `python3 scripts/check_release_metadata_versions.py`, focused `ctest --test-dir build -R '^test_edge_cases$' --output-on-failure`, and `bash scripts/unix/run_tests.sh` (`73/73`); all passed.
- Closed GitHub issue `#119` after local verification.
- Started issue `#98`. Added `CONTRIBUTING.md` with repository requirements, CMake build/test commands, the focused Unix test script, code style expectations, release/history documentation notes, and PR checklist guidance. Linked it from the README and advanced release/history metadata to `1.3.25`. This is docs-only and does not affect runtime behavior or complexity.
- Verified issue `#98` locally with `git diff --check` and `python3 scripts/check_release_metadata_versions.py`; both passed.
- Closed GitHub issue `#98` after local verification.
- Started issue `#96`. Added root `VERSIONING.md` with the repository SemVer policy for stable release tags, linked it from `README.md` and `docs/README.md`, and advanced release/history plus package metadata to `1.3.30`. This is documentation and release metadata only; no runtime behavior or complexity changes.
- Verified issue `#96` locally with `git diff --check` and `python3 scripts/check_release_metadata_versions.py`; both passed. Closed GitHub issue `#96` after verification.
- Started issue `#92`. Added a general graph thread-safety policy to `README.md` and `docs/API_REFERENCE.md`, documenting read-only sharing, external synchronization requirements for mutation, and staged-flow serialization per graph instance. Advanced release/history metadata to `1.3.31`. This is documentation only and does not affect runtime behavior or complexity.
- Verified issue `#92` locally with `git diff --check` and `python3 scripts/check_release_metadata_versions.py`; both passed. Closed GitHub issue `#92` after verification.
- Started issue `#134`. Added explicit 30-second `TIMEOUT` properties to each CTest target registered by `CMakeLists.txt`, covering the formal/smoke test loop and the large-graph comparison target. Advanced release/history metadata to `1.3.32`. This is CI/test configuration only and does not affect library runtime behavior or algorithmic complexity.
- Verified issue `#134` locally with `git diff --check`, `python3 scripts/check_release_metadata_versions.py`, CMake configure, and `ctest --show-only=json-v1` metadata parsing; all configured CTest targets expose `TIMEOUT=30`. Closed GitHub issue `#134` after verification.
- Started issue `#137`. Factored the shared `bfs_tree(...)` and `dfs_tree(...)` tree-construction loop into a small traversal helper while preserving validation, returned graph type, and `O(n + m)` behavior. Advanced release/history metadata to `1.3.33`.
- Verified issue `#137` locally with `git diff --check`, release metadata check, focused `test_edge_cases` rebuild, and focused `ctest`; all passed. Closed GitHub issue `#137` after verification.
- Started issue `#135`. Added private `tests/test_helpers.hpp` for shared C++ test expectations, exception checks, colored output, and automatic test-case counting. Converted the existing C++ test executables to list-based `run_tests(...)` calls and advanced release/history metadata to `1.3.34`. This is test infrastructure only and does not affect library runtime behavior or algorithmic complexity.
- Verified issue `#135` locally with `git diff --check`, release metadata check, focused rebuilds of the touched test targets, `bash scripts/unix/run_tests.sh` (`73/73`), and full CTest (`8/8`, including `test_large_graph_compare`). Closed GitHub issue `#135` after verification and prepared the change for direct push to `main`.
- Started issue `#127`. Extracted shared internal capacity and weighted flow-graph construction helpers in `include/nxpp/flow.hpp`, reusing them across max-flow, min-cut, staged cycle-canceling, and successive-shortest-path setup. Advanced release/history metadata to `1.3.35`. The refactor keeps the same `O(n + m)` flow setup work and does not change public API or runtime behavior.
- Verified issue `#127` locally with `git diff --check`, release metadata check, focused `test_flow` build and CTest, `bash scripts/unix/run_tests.sh` (`73/73`), full CTest (`8/8`), and single-header tests (`70/70`). Closed GitHub issue `#127` after verification and prepared the change for direct push to `main`.
- Started issue `#133`. Added the large-graph comparison script to `.github/workflows/release.yml` before workflow-dispatch tag creation and GitHub release publication, and advanced release/history metadata to `1.3.36`. This is release CI configuration only and does not affect library runtime behavior or algorithmic complexity.
- Verified issue `#133` locally with `git diff --check`, release metadata check, and `bash scripts/unix/run_large_graph_compare.sh`; all passed. Closed GitHub issue `#133` after verification.
- Started issue `#100`. Added a dedicated `.github/workflows/sanitizers.yml` workflow that builds the formal CMake test suite on Linux/GCC with ASan and UBSan, then runs CTest with fail-fast sanitizer options. Advanced release/history metadata to `1.3.37`. This is CI configuration only and does not affect library runtime behavior or algorithmic complexity.
- Verified issue `#100` locally with `git diff --check`, release metadata check, sanitizer CMake configuration, and focused `test_edge_cases` build/CTest under ASan and UBSan; all passed. Closed GitHub issue `#100` after verification.
- Followed up on issue `#100` after CI showed that downloaded Boost headers were not enough for CMake `find_package(Boost)`. Changed the sanitizer workflow to install `libboost-graph-dev` through apt and removed the manual Boost include flag.
- Followed up on the compatibility workflow after the macOS Boost index parser failed on non-UTF-8 bytes from `archives.boost.io`. Made the shared Unix parser decode stdin bytes with UTF-8 error ignoring, covering ubuntu-gcc, ubuntu-clang, macos-clang, and the large-graph comparison job.
- Started issue `#128`. Added focused assertion-based coverage across the listed untested areas: topological sort success/failure, Kruskal MST, DAG shortest paths, 2-SAT satisfiability, batch node/edge insertion, graph copy/move independence, undirected unweighted subgraph extraction, DOT write failures, directed betweenness normalization, edge-id try-get attributes, and missing edge-id removal. Advanced release/history metadata to `1.3.38`. This is test coverage only and does not affect library runtime behavior or algorithmic complexity.
- Verified issue `#128` locally with `git diff --check`, release metadata check, focused CTest for the touched test targets, the full C++ test script (`84/84`), and single-header tests (`80/80`); all passed. Closed GitHub issue `#128` after verification.
- Started issue `#99`. Marked `Graph::num_vertices()` and `Graph::num_edges()` as `noexcept` and added compile-time coverage for that contract. Left predicate methods that depend on user-provided node comparison unchanged because their exception guarantees can depend on `NodeID`. Advanced release/history metadata to `1.3.39`. This is API contract annotation only and does not affect runtime behavior or algorithmic complexity.
