...
- Closed issue `#86` by documenting the `remove_node` hot path: added a Doxygen `@warning` to `include/nxpp/graph.hpp`, added a node-removal/performance section to `docs/API_REFERENCE.md` with an anti-pattern example and a rebuild-based alternative, and added a one-line quick-start pitfall link in the root `README.md`. Aligned the core mutation table row and release/history docs to `1.0.44` in `CHANGELOG.md` and `RELEASE_NOTES.md`.
