...
| `num_edges()` | `boost::num_edges` | `O(1)` | `O(1)` | `yes` | Delegates directly to Boost's maintained edge count and avoids the `O(m)` vector materialization cost of `edges().size()`. |
