...
assert.deepEqual(toSortedNumbers(graphInt.nodes()), [1, 2, 3, 4], "GraphInt nodes() must preserve numeric IDs");
assert.deepEqual(toSortedNumbers(graphInt.neighbors(1)), [2], "GraphInt neighbors() must preserve undirected adjacency");
const graphIntSubgraph = graphInt.subgraph([1, 2, 3]);
assert.deepEqual(toSortedNumbers(graphIntSubgraph.nodes()), [1, 2, 3], "GraphInt subgraph() must copy selected nodes");
assert.equal(graphIntSubgraph.hasEdge(1, 2), true, "GraphInt subgraph() must keep induced edges");
assert.equal(graphIntSubgraph.hasEdge(2, 4), false, "GraphInt subgraph() must omit edges leaving the selection");
assert.equal(graphIntSubgraph.getEdgeWeight(1, 2), 3, "GraphInt subgraph() must preserve edge weights");
assertThrows(() => graphInt.subgraph([1, 99]), "GraphInt subgraph() must reject missing nodes");
...
diStr.setNodeAttr("A", "label", "source");
diStr.setEdgeAttr("A", "B", "kind", "directed");
...
const diStrSubgraph = diStr.subgraph(["A", "B"]);
assert.deepEqual(toSortedStrings(diStrSubgraph.nodes()), ["A", "B"], "DiGraphStr subgraph() must copy selected nodes");
assert.equal(diStrSubgraph.hasEdge("A", "B"), true, "DiGraphStr subgraph() must keep directed induced edges");
assert.equal(diStrSubgraph.hasEdge("B", "C"), false, "DiGraphStr subgraph() must omit outgoing edges to omitted nodes");
assert.equal(diStrSubgraph.getNodeAttr("A", "label"), "source", "DiGraphStr subgraph() must copy node attributes");
assert.equal(diStrSubgraph.getEdgeAttr("A", "B", "kind"), "directed", "DiGraphStr subgraph() must copy edge attributes");
