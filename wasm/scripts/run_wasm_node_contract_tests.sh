...
MODULE_PATH="$ROOT_DIR/.tmp/wasm-node/nxpp_node.mjs"
OUTPUT_FILE="$ROOT_DIR/.tmp/wasm-node/node_api_contract_test.out"
...
set +e
NXPP_WASM_NODE_MODULE="$MODULE_PATH" "$NODE_BIN" "$ROOT_DIR/wasm/node/node_api_contract_test.mjs" > "$OUTPUT_FILE" 2>&1
status=$?
set -e
cat "$OUTPUT_FILE"
