...
- Closed `#63` by preparing and validating a first local vcpkg overlay port under `packaging/vcpkg/ports/nxpp`: the repository now includes `portfile.cmake` and `vcpkg.json` for local overlay-port consumption on top of the installed CMake package path, the port removes the empty `lib/` / `debug/` directories that vcpkg flags in post-build validation, and both `vcpkg install nxpp --overlay-ports=...` and a small `find_package(nxpp CONFIG REQUIRED)` consumer path have now been validated locally.
