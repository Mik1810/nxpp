...
- Closed `#62` by adding a first local Conan recipe: the repository now includes `conanfile.py` that models `nxpp` as a header-only package, declares Boost as a Conan dependency, and documents `conan create .` as the intended first local validation step before any remote publication workflow is considered.
