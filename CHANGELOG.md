...
- Added a generated Embind declaration and mechanical raw-contract check for
  WASM constructors, method arity, refined TypeScript types, and class parity.
...
- Documented how snapshot changes should be reviewed against the repository versioning policy when intentional public API changes occur.
...
- Preserved the existing max-flow, min-cut, staged cycle-canceling, and successive-shortest-path behavior while keeping flow setup `O(n + m)`.
