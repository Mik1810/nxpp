...
- Do not dump large JSON, logs, issue bodies, comments, generated files, build dirs, lockfiles, or dependency trees.
...
- If context grows large, stop after the current task and recommend a fresh session with a short handoff.
...
timeout 30s tail -40 SESSION.md
...
- If touching WASM bindings or TypeScript facade, run or propose:
...
- Benchmark runs that generate CSV files are user-driven only.
