...
usage() {
  cat <<'EOF'
Usage:
  scripts/unix/benchmark_snippet_compile.sh [options]
  scripts/unix/benchmark_snippet_compile.sh <snippet_folder|all> [iterations]

Examples:
  scripts/unix/benchmark_snippet_compile.sh --snippet mcmf_cc
  scripts/unix/benchmark_snippet_compile.sh --snippet mcmf_cc --iterations 5
  scripts/unix/benchmark_snippet_compile.sh --snippet mcmf_cc --opt-level O0
  scripts/unix/benchmark_snippet_compile.sh --all --iterations 3 --csv benchmark/compilation_snippet.csv
  scripts/unix/benchmark_snippet_compile.sh mcmf_cc 5
  scripts/unix/benchmark_snippet_compile.sh all 3
...
Options:
  ...
  --verbose              Print detailed per-snippet headers, timings, and summaries
...
if [[ -z "$TIME_BIN" ]]; then
  echo "ERROR: GNU time not found. Install the 'time' package or provide 'gtime'." >&2
  exit 1
fi
...
compile_seconds() {
  ...
  if ! "$TIME_BIN" -f "%e" -o "$timing_file" \
    g++ -std=c++20 $CXXFLAGS_OVERRIDE "$source_file" -I"$ROOT_DIR" -o "$output_file" \
    2>"$stderr_file"; then
    echo "ERROR: compilation failed for $source_file" >&2
  ...
  if [[ -z "$seconds" ]]; then
    echo "ERROR: failed to capture compile time for $source_file using $TIME_BIN" >&2
    sed -n '1,80p' "$stderr_file" >&2
    rm -f "$timing_file" "$stderr_file"
    return 1
  fi
...
benchmark_one() {
  ...
  for ((i = 1; i <= ITERATIONS; ++i)); do
    ...
    if [[ "$VERBOSE" == "1" ]]; then
      ...
      printf "SNIPPET: %s | RUN: %d/%d\n" "$snippet_name" "$i" "$ITERATIONS"
    fi
  done
