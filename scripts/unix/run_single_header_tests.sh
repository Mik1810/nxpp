...
BUILD_DIR="$ROOT_DIR/.tmp/single-header-tests"
mkdir -p "$BUILD_DIR"

CXX=${CXX:-g++}
CXXFLAGS=${CXXFLAGS:-"-std=c++20 -Wall -Wextra -pedantic -O0"}
HEADER_MACRO='-DNXPP_HEADER_UNDER_TEST="dist/nxpp.hpp"'
GREEN=$'\033[32m'
RED=$'\033[31m'
RESET=$'\033[0m'
...
run_test_binary() {
    ...
    echo "Testing $display_name against dist/nxpp.hpp"

    $CXX $CXXFLAGS $HEADER_MACRO "$ROOT_DIR/$source_file" -I"$ROOT_DIR" -o "$test_bin"
    set +e
...
run_test_binary "tests/test_api_compat.cpp"
...
if [ "$overall_status" -eq 0 ]; then
    echo "[TEST] All single-header tests passed (${passed_tests}/${total_tests}) | ${GREEN}PASS${RESET}"
    exit 0
fi

echo "[TEST] All single-header tests passed (${passed_tests}/${total_tests}) | ${RED}FAIL${RESET}"
exit 1
