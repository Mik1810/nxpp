...
param(
    [switch]$Smoke,
    [string]$HeaderUnderTest = "dist/nxpp.hpp"
)

. "$PSScriptRoot/common.ps1"

$root = Get-NxppRoot
$buildDir = Join-Path $root ".tmp\single-header-tests-windows"
...
foreach ($sourceFile in $sources) {
    ...
    Write-Output "Testing $displayName against $HeaderUnderTest"

    $compileResult = Invoke-NxppCompileSource -SourceFile $sourceFile -OutputFile $testBin -Optimization "O0" -Compiler $compiler -FlagsOverride $flagsOverride -Definitions $definitions
...
if ($overallStatus -eq 0) {
    Write-Output "[TEST] All single-header tests passed ($passedTests/$totalTests) | PASS"
    exit 0
}

Write-Output "[TEST] All single-header tests passed ($passedTests/$totalTests) | FAIL"
exit 1
