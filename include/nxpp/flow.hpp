...
template <typename T>
std::optional<long> try_flow_capacity_any_cast(const std::any& value) {
...
long Graph<NodeID, EdgeWeight, Directed, Multi, Weighted, OutEdgeSelector, VertexSelector>::get_edge_flow_capacity(
    std::size_t edge_id,
    const std::string& key) const {
    if (key == "weight") {
        if constexpr (Weighted) {
            return detail::checked_flow_capacity_value(get_edge_weight(edge_id));
        } else {
    ...
    if (auto capacity = detail::try_flow_capacity_any_cast<int>(value)) return *capacity;
    if (auto capacity = detail::try_flow_capacity_any_cast<long>(value)) return *capacity;
    if (auto capacity = detail::try_flow_capacity_any_cast<long long>(value)) return *capacity;
    if (auto capacity = detail::try_flow_capacity_any_cast<unsigned int>(value)) return *capacity;
    if (auto capacity = detail::try_flow_capacity_any_cast<unsigned long>(value)) return *capacity;
    if (auto capacity = detail::try_flow_capacity_any_cast<unsigned long long>(value)) return *capacity;
    if (auto capacity = detail::try_flow_capacity_any_cast<float>(value)) return *capacity;
    if (auto capacity = detail::try_flow_capacity_any_cast<double>(value)) return *capacity;
    if (auto capacity = detail::try_flow_capacity_any_cast<long double>(value)) return *capacity;
